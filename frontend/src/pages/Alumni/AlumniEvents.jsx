...
const AlumniEvents = () => {
  ...
  const normalizeEvent = (event) => ({
    ...
    title: event.title || '',
    type: event.event_type || event.type || 'Reunion',
    ...
    registered: event.registered_count || event.registered || 0,
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const rows = res.data?.events || res.data || [];
        setAlumniEvents((Array.isArray(rows) ? rows : []).map(normalizeEvent));
  ...
  const handleSubmitRegistration = async () => {
    ...
    try {
      await alumniAPI.registerForEvent(selectedEvent.id);
      ...
      // Refresh events
      const res = await alumniAPI.getEvents();
      setAlumniEvents(res.data?.events || res.data || []);
    } catch (e) {
      showSnackbar(e.response?.data?.detail || 'Registration failed', 'error');
    }
  };
  ...
  const handleSubmitEvent = async () => {
    ...
    try {
      const payload = {
        ...
        fee: Number(eventFormData.fee) || 0,
        organizer: '',
      };
      ...
      if (eventCoverFile) {
        ...
        formData.append('cover_image_file', eventCoverFile);
        await alumniAPI.createEvent(formData);
      } else {
        await alumniAPI.createEvent(payload);
      }
      ...
      showSnackbar(e.response?.data?.detail || 'Event creation failed', 'error');
    }
  };
        ...
        {upcomingEvents.length === 0 ? (
          ...
          >
            {upcomingEvents.map((event) => {
              ...
              return (
                <Grid item xs={12} md={6} key={event.id} component={motion.div} variants={fadeInUp}>
                  <Card
                    sx={{
                      height: '100%',
                    ...
                    {event.coverImage ? (
                      ...
                      <Box
                        sx={{
                          ...
                          background: 'linear-gradient(135deg, rgba(25,118,210,0.18), rgba(25,118,210,0.04))',
                        }}
                      >
                        <Event sx={{ fontSize: 64, color: 'primary.main', opacity: 0.7 }} />
                      </Box>
                    )}
                    <CardContent>
                      {/* Header */}
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                ...
                <FileDropzone
                  acceptedTypes="image/*"
                  maxSize={5}
                  onFileSelect={(file) => setEventCoverFile(file)}
                />
              </Grid>
            </Grid>
          </DialogContent>
