...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const res = await analyticsAPI.getFacultyDashboard();
        const d = res.data;
        const sections = Array.isArray(d?.sections) ? d.sections : [];
        const totalStudents = Number(d?.total_students || 0);
        const avgAttendance = sections.length
          ? sections.reduce((sum, s) => sum + Number(s.avg_attendance_pct || 0), 0) / sections.length
          : 0;
        const avgAssignment = sections.length
          ? sections.reduce((sum, s) => sum + Number(s.avg_assignment_score || 0), 0) / sections.length
          : 0;
        const classAverage = sections.length
          ? sections.reduce((sum, s) => sum + Number(s.avg_quiz_score || 0), 0) / sections.length
          : 0;

        setStats([
          { title: 'Total Students', value: String(totalStudents), subtitle: `${d?.total_sections || 0} sections`, icon: People, color: 'primary', tooltip: '' },
          { title: 'Avg Attendance', value: `${avgAttendance.toFixed(1)}%`, subtitle: 'Across all sections', icon: CheckCircle, color: 'success', tooltip: '' },
          { title: 'Assignment Completion', value: `${avgAssignment.toFixed(1)}%`, subtitle: 'Avg section score', icon: Assignment, color: 'warning', tooltip: '' },
          { title: 'Class Average', value: `${classAverage.toFixed(1)}%`, subtitle: 'Avg quiz score', icon: School, color: 'info', tooltip: '' },
        ]);

        const gradeBuckets = { A: 0, B: 0, C: 0, D: 0, F: 0 };
        sections.forEach((s) => {
          const score = Number(s.avg_assignment_score || 0);
        ...
        setAttendanceTrend(
          sections.map((s, idx) => ({
            week: s.course_name || `Section ${idx + 1}`,
            attendance: Number(s.avg_attendance_pct || 0),
          }))
        );
        ...
        const perfRows = [
          { name: 'Excellent', value: sections.filter((s) => Number(s.avg_assignment_score || 0) >= 85).length, color: '#4caf50' },
          { name: 'Good', value: sections.filter((s) => Number(s.avg_assignment_score || 0) >= 70 && Number(s.avg_assignment_score || 0) < 85).length, color: '#2196f3' },
          { name: 'Average', value: sections.filter((s) => Number(s.avg_assignment_score || 0) >= 55 && Number(s.avg_assignment_score || 0) < 70).length, color: '#ff9800' },
          { name: 'Needs Support', value: sections.filter((s) => Number(s.avg_assignment_score || 0) < 55).length, color: '#f44336' },
        ];
        ...
        setWeeklyProgress(
          sections.map((s, idx) => ({
            week: `W${idx + 1}`,
            avgScore: Number(s.avg_assignment_score || 0),
            attendance: Number(s.avg_attendance_pct || 0),
          }))
        );
        ...
        setCourseComparison(
          sections.map((s) => ({
            ...
            avgGrade: Number((Number(s.avg_assignment_score || 0) / 25).toFixed(2)),
            passRate: Number(s.avg_assignment_score || 0),
          }))
        );
