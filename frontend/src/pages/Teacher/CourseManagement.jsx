...
const CourseManagement = () => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        const [courseRes, assignRes, matRes, annRes] = await Promise.allSettled([
          sisAPI.getCourse(id),
        ...
        if (courseRes.status === 'fulfilled') {
          ...
          setCourse(c);
          setStudents(c?.enrolled_students || c?.students_list || []);
        }
        if (assignRes.status === 'fulfilled') setAssignments(assignRes.value.data?.assignments || assignRes.value.data || []);
        if (matRes.status === 'fulfilled') setMaterials(matRes.value.data?.materials || matRes.value.data || []);
