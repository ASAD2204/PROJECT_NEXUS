...
const TeacherCourses = () => {
  ...
  const avgAttendance = courses.length > 0 
    ? (courses.reduce((sum, c) => sum + Number(c.attendance || c.attendance_percentage || 0), 0) / courses.length).toFixed(1) + '%' 
    : '0%';
  const pendingAssignments = courses.reduce((sum, c) => sum + Number(c.pendingAssignments || c.pending_assignments || 0), 0);
    ...
    const fetchCourses = async () => {
      try {
        ...
        const normalized = (Array.isArray(rows) ? rows : []).map((c) => ({
          ...
          name: c.course?.title || c.title || c.name || `Section ${c.section_id}`,
          code: c.course?.code || c.code || `SEC-${c.section_id}`,
          semester: c.semester_id ? `Semester ${c.semester_id}` : 'N/A',
          creditHours: Number(c.course?.credit_hours ?? c.credit_hours ?? c.creditHours ?? 0),
          students: c.enrolled_students ?? c.students ?? 0,
          schedule: c.schedule || 'TBA',
          room: c.room_no || c.room || 'TBA',
          status: c.status || 'ongoing',
          assignments: c.assignments_count ?? c.assignments ?? 0,
          quizzes: c.quizzes_count ?? c.quizzes ?? 0,
          avgGrade: c.avg_grade != null ? `${c.avg_grade}%` : '-',
          attendance: Number(c.attendance_percentage ?? c.attendance ?? 0),
        }));
        setCourses(normalized.filter(c => c.status !== 'completed'));
        setArchivedCourses(normalized.filter(c => c.status === 'completed'));
