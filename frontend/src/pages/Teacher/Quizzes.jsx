  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        ...
        const totalAttempts = data.reduce((s, q) => s + (q.attempts || 0), 0);
        const avgScore = data.length ? Math.round(data.reduce((s, q) => s + (q.avgScore || q.avg_score || 0), 0) / data.length) : 0;
        setStats([
          { title: 'Total Quizzes', value: String(data.length), icon: QuizIcon, color: 'primary.main', tooltip: 'Total quizzes created' },
