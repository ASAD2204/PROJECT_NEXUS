...
const CreateQuiz = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        setCourses(crs.map(c => ({ value: c.code || c.id, label: `${c.code || c.id} - ${c.name}` })));
  ...
  const handleSaveDraft = async () => {
    try {
      await lmsAPI.createQuiz({ ...quizInfo, questions, status: 'draft' });
    } catch (e) { console.error(e); }
  ...
  const handlePublish = async () => {
    ...
    try {
      await lmsAPI.createQuiz({ ...quizInfo, questions, status: 'active' });
    } catch (e) { console.error(e); }
    navigate('/teacher/quizzes');
  };
