...
const ViewSubmissions = () => {
  ...
  const [content, setContent] = useState({ id, title: '', course: '', courseCode: '', dueDate: '', totalMarks: 0, description: '' });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (isQuiz) {
          const [quizRes, subRes] = await Promise.allSettled([
            lmsAPI.getQuiz(id),
            lmsAPI.getSubmissions(id, { type: 'quiz' }),
          ]);
          if (quizRes.status === 'fulfilled') setContent(quizRes.value.data || {});
          ...
          const [assignRes, subRes] = await Promise.allSettled([
            lmsAPI.getAssignment(id),
            lmsAPI.getSubmissions(id),
          ]);
          if (assignRes.status === 'fulfilled') setContent(assignRes.value.data || {});
          if (subRes.status === 'fulfilled') setSubmissions(subRes.value.data?.submissions || subRes.value.data || []);
        }
  ...
  const handleSubmitGrade = async () => {
    try {
      await lmsAPI.gradeSubmission(selectedSubmission.id, { marks: Number(marks), feedback });
      setSubmissions(prev => prev.map(s => s.id === selectedSubmission.id ? { ...s, marksObtained: Number(marks), feedback, status: 'graded' } : s));
    } catch (e) { console.error('Failed to grade', e); }
                  ...
                  <TableCell>
                    {submission.status === 'graded' ? (
                      <Stack spacing={0.5}>
                        ...
                        <LinearProgress 
                          variant="determinate" 
                          value={(submission.marksObtained / (content.totalMarks || 1)) * 100}
