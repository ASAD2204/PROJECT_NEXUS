...
const GrievanceManagement = () => {
  ...
  const normalizeGrievance = (g) => ({
    id: g.id || g.ticket_id,
    ticketId: g.ticketId || `GRV-${g.ticket_id}`,
    category: g.category || 'General',
    subject: g.subject || 'Untitled grievance',
    description: g.description || '',
    status: g.status || 'Pending',
    priority: g.priority || (g.is_urgent ? 'High' : 'Normal'),
    submittedAt: g.submittedAt || g.created_at || new Date().toISOString(),
    studentName: g.student_name || 'Student',
    studentRollNo: g.student_roll_no || 'N/A',
    comments: (g.comments || []).map(c => ({
      id: c.comment_id,
      author: c.author_name || 'User',
      avatar: c.author_avatar,
      role: c.author_role || 'Staff',
  ...
  const loadGrievances = useCallback(async () => {
    try {
      ...
      const normalized = (Array.isArray(data) ? data : []).map(normalizeGrievance);
  ...
  const handleResolve = async () => {
    if (!selected) return;
    try {
      await opsAPI.updateGrievanceStatus(selected.id, { status: 'Resolved' });
      showSnackbar(`Marked ${selected.ticketId} as resolved`, 'success');
      await loadGrievances();
    } catch {
      showSnackbar('Failed to update grievance status', 'error');
    }
  };
  ...
  const handleEscalate = async () => {
    if (!selected) return;
    try {
      await opsAPI.updateGrievanceStatus(selected.id, { status: 'In Progress', resolution: 'Escalated by admin' });
      showSnackbar(`Escalated ${selected.ticketId} to higher authority`, 'warning');
      await loadGrievances();
    } catch {
      showSnackbar('Failed to escalate grievance', 'error');
    }
  };
  ...
  const handleReply = async () => {
    ...
    try {
      await opsAPI.addGrievanceComment(selected.id, replyText.trim());
      await opsAPI.updateGrievanceStatus(selected.id, { status: 'In Progress' });
              ...
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3 }}>
                  <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 3 }}>
                    ...
                    >
                      {selected.studentName ? selected.studentName[0] : <Person sx={{ fontSize: 32 }} />}
                    </Avatar>
                    <Box sx={{ flexGrow: 1 }}>
                      ...
                      <Stack direction="row" spacing={1} alignItems="center">
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Roll No: {selected.studentRollNo} • {selected.ticketId}
                        </Typography>
                      </Stack>
                    </Box>
