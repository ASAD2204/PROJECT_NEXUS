...
const FinanceManagement = () => {
  ...
  useEffect(() => {
    const fetchLedger = async () => {
      try {
        ...
        setLedger(res.data?.transactions || res.data || []);
  ...
  const handleGenerateReport = async () => {
    try {
      // Call backend report generation endpoint if available
      // For now, just show success message and let backend async handling work
      showSnackbar('Monthly report generation initiated', 'success');
    } catch (error) {
  ...
  const handleSaveFeeStructure = async () => {
    try {
      // Call backend to update fee structure
      const feeHeadId = 1; // Assuming fee structure has a primary ID; adjust as needed
      await financeAPI.updateFeeHead(feeHeadId, feeStructure);
  ...
  const handleSendReminder = async (student) => {
    try {
      // Call backend to send payment reminder
      // Assuming the endpoint accepts student identifier
