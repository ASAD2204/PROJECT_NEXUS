...
const CourseManagement = () => {
  ...
  const normalizeCourse = useCallback((course) => ({
    ...
    department: course.department || `Dept ${course.dept_id ?? '-'}`,
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const deptRows = deptsRes.status === 'fulfilled'
        ? (deptsRes.value.data?.departments || deptsRes.value.data || [])
        : [];

      setCourses((Array.isArray(courseRows) ? courseRows : []).map(normalizeCourse));
  ...
  const handleSaveCourse = async () => {
    if (!formData.courseCode || !formData.courseTitle || !formData.creditHours) return;

    const payload = {
      code: formData.courseCode,
      title: formData.courseTitle,
      credit_hours: Number(formData.creditHours),
      dept_id: Number(formData.department) || 1,
      description: formData.description || null,
    };

    try {
      if (editingCourseId) {
        await sisAPI.updateCourse(editingCourseId, payload);
      } else {
        await sisAPI.createCourse(payload);
      }
      setOpenDialog(false);
      resetForm();
      await loadData();
