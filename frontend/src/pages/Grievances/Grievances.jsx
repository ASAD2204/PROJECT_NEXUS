...
const Grievances = () => {
  ...
  const stats = useMemo(() => {
    ...
    const pending = grievances.filter((g) => g.status === 'Pending' || g.status === 'In Progress' || g.status === 'Submitted').length;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showSnackbar(`Ticket #${result.ticketId || result.ticket_id || 'NEW'} created successfully`, 'success');
      fetchGrievances();
    } catch {
      showSnackbar('Failed to submit grievance', 'error');
    }
    setOpen(false);
  ...
  const handleReply = async (ticketId) => {
    ...
    try {
      await studentAPI.addTicketReply(ticketId, comment);
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3 }} component={motion.div} variants={fadeInUp}>
            <StatCard
              ...
              value={(() => {
                ...
                const avgDays = resolved.reduce((sum, g) => {
                  const created = new Date(g.created_at || g.createdAt || Date.now());
                  const updated = new Date(g.updated_at || g.updatedAt || Date.now());
                            ...
                            <Stack direction="row" spacing={1.5} alignItems="flex-start">
                              <Avatar 
                                ...
                                sx={{ 
                                  width: 32, 
                                  height: 32,
                                  bgcolor: msg.from === 'You' ? 'primary.main' : 'success.main',
                                  fontSize: '0.875rem'
                                }}
                              >
                                {msg.from?.[0] || 'U'}
                              ...
                              <Box sx={{ flexGrow: 1 }}>
                                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.5 }}>
                                  <Typography 
                                    variant="subtitle2" 
                                    fontWeight="bold" 
                                    color={msg.from === 'You' ? 'primary.main' : 'success.main'}
                                  >
                                    {msg.from}
                                  </Typography>
                      ...
                      <Box sx={{ mt: 3 }}>
                        ...
                        <Stack direction="row" spacing={2}>
                          ...
                          <Button
                            variant="contained"
                            endIcon={<Send />}
                            onClick={() => handleReply(g.ticket_id || g.id)}
                            sx={{
                              minWidth: 120,
                              borderRadius: 2,
                              background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)',
