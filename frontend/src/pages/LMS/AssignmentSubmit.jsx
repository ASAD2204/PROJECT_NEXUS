...
const AssignmentSubmit = () => {
  ...
  const returnToCourse = () => {
    const courseId = location.state?.courseId || assignment?.courseId || assignment?.course_id || course?.id || course?.section_id;
    if (courseId) {
  ...
  const normalizeAssignment = (row) => {
    ...
    const totalMarks = row.totalMarks ?? row.total_marks ?? 0;
    const obtainedMarks = row.obtainedMarks ?? row.obtained_marks ?? null;
    const submittedAt = row.submittedAt ?? row.submitted_at ?? null;
    const rawStatus = String(row.status || row.submission_status || '').toLowerCase();
    let status = 'Pending';
    if (obtainedMarks !== null && obtainedMarks !== undefined || rawStatus === 'graded') status = 'Graded';
    else if (submittedAt || rawStatus === 'submitted') status = 'Submitted';
    ...
    return {
      ...
      assignment_id: row.assignment_id || row.id,
      dueDate,
      due_date: dueDate,
      totalMarks,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const stateAssignment = normalizeAssignment(location.state?.assignment || null);
        if (stateAssignment) {
          setAssignment(stateAssignment);
        }

        const res = await lmsAPI.getAssignment(id);
        const a = normalizeAssignment(res.data);
        setAssignment(a);
        try {
          const subRes = await lmsAPI.getMySubmissions();
          const all = Array.isArray(subRes.data)
            ? subRes.data
            : (subRes.data?.submissions || []);
          const filtered = all.filter(
            (item) => String(item.assignment_id || item.assignmentId) === String(id)
          );
        ...
        if (a?.courseId || a?.course_id) {
          try {
            const cRes = await import('../../api/sis').then(m => m.sisAPI.getCourse(a.courseId || a.course_id));
            setCourse(cRes.data);
          } catch { /* optional */ }
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setShowSuccess(true);
      setTimeout(() => { returnToCourse(); }, 3000);
    } catch (e) {
      console.error('Submission failed', e);
      setIsUploading(false);
      setUploadProgress(0);
      setFileError(e.response?.data?.detail || 'Submission failed. Please try again.');
    }
  };
          ...
          <Stack spacing={3}>
            ...
            <Card>
              <CardContent>
                ...
                <Chip 
                  label={assignment.course_code ? `${assignment.course_code} - ${assignment.course_title}` : `${course?.code || 'COURSE'} - ${course?.title || ''}`} 
                  color="primary" 
                  sx={{ mb: 2 }}
                />
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                  <Avatar src={course?.instructorPhoto}>
                    {(assignment.instructor_name || course?.instructor || 'I')[0]}
                  </Avatar>
                  <Box>
                    ...
                    <Typography variant="body1" fontWeight="bold">
                      {assignment.instructor_name || course?.instructor || 'Course Faculty'}
                    </Typography>
                  </Box>
                </Box>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Typography variant="body1">
                    {assignment.postedOn || assignment.created_at ? new Date(assignment.postedOn || assignment.created_at).toLocaleDateString() : 'Recently'}
                  </Typography>
                </Box>
              ...
              {assignment.status === 'Submitted' && (
                <Box>
                  ...
                  <Box sx={{ mb: 3 }}>
                    ...
                    <Typography variant="h6" fontWeight="bold">
                      {assignment.submittedOn || previousSubmissions[0]?.submitted_at ? new Date(assignment.submittedOn || previousSubmissions[0]?.submitted_at).toLocaleString() : 'Recently'}
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ mb: 3 }}>
                    ...
                    <List>
                      {(assignment.submittedFiles || previousSubmissions[0]?.files || [{name: 'Submitted_File.pdf', size: 'Unknown size'}]).map((file, idx) => (
                        <ListItem key={idx} sx={{ backgroundColor: 'action.hover', borderRadius: 2, mb: 1 }}>
                          <ListItemIcon>
                            <InsertDriveFile color="primary" />
                          </ListItemIcon>
                          <ListItemText
                  ...
                  >
                    <CardContent sx={{ textAlign: 'center', py: 4 }}>
                      ...
                      <Typography variant="h4" sx={{ color: 'white', mt: 1, textShadow: '0 1px 4px rgba(0,0,0,0.2)' }}>
                        {Math.round((animatedGrade / (assignment.totalMarks || 1)) * 100)}%
                      </Typography>
                      <Chip
                        label={
                          (animatedGrade / (assignment.totalMarks || 1)) >= 0.9 ? 'A' :
                          (animatedGrade / (assignment.totalMarks || 1)) >= 0.8 ? 'B' :
                          (animatedGrade / (assignment.totalMarks || 1)) >= 0.7 ? 'C' :
                          (animatedGrade / (assignment.totalMarks || 1)) >= 0.6 ? 'D' : 'F'
                        }
                        sx={{
                          mt: 2,
                          fontSize: '1.5rem',
                        ...
                        >
                          <ListItemText
                            ...
                            secondary={
                              <>
                                <Typography variant="caption" display="block">
                                  Submitted: {sub.submittedAt || sub.submitted_at || 'N/A'}
                                </Typography>
                                <Typography variant="caption" display="block">
                                  Files: {Array.isArray(sub.files) ? sub.files.join(', ') : 'N/A'}
                                </Typography>
                              </>
                            }
                          />
                          <Chip label={sub.grade || sub.score || 'Submitted'} size="small" color="primary" />
                        </ListItem>
                      ))}
                    </List>
                  </AccordionDetails>
