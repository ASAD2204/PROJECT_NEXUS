...
const CourseClassroom = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [courseRes, assignRes, quizRes, annRes] = await Promise.allSettled([
          sisAPI.getCourse(id),
          lmsAPI.getCourseAssignments(id),
          lmsAPI.getCourseQuizzes(id),
          lmsAPI.getCourseAnnouncements(id),
        ]);
        if (courseRes.status === 'fulfilled') setCourse(courseRes.value.data);
        if (assignRes.status === 'fulfilled') setCourseAssignments(assignRes.value.data?.assignments || assignRes.value.data || []);
        if (quizRes.status === 'fulfilled') setCourseQuizzes(quizRes.value.data?.quizzes || quizRes.value.data || []);
        if (annRes.status === 'fulfilled') setCourseAnnouncements(annRes.value.data?.announcements || annRes.value.data || []);
      } catch { /* empty */ }
      setLoading(false);
    };
    fetchData();
  ...
  const enrolledStudents = course?.enrolled_students || course?.enrolledStudents || 0;
  ...
  const gradeRows = [
    ...(courseAssignments || []).map((a) => ({
      ...
      category: 'Assignment',
      earned: a.earned ?? a.obtained_marks ?? 0,
      total: a.total ?? a.total_marks ?? 100,
      percentage: a.percentage ?? (a.total_marks ? Math.round(((a.obtained_marks || 0) / a.total_marks) * 100) : 0),
      weight: a.weight ?? 10,
    })),
    ...(courseQuizzes || []).map((q) => ({
      name: q.title || q.name || 'Quiz',
      category: 'Quiz',
      earned: q.earned ?? q.obtained_marks ?? 0,
      total: q.total ?? q.total_marks ?? 100,
      percentage: q.percentage ?? (q.total_marks ? Math.round(((q.obtained_marks || 0) / q.total_marks) * 100) : 0),
      weight: q.weight ?? 5,
    })),
  ];
  const gradeData = gradeRows.length ? gradeRows : [{ name: 'No graded items yet', category: 'N/A', earned: 0, total: 0, percentage: 0, weight: 0 }];

  const finalGrade = gradeRows.length
    ? Math.round(gradeRows.reduce((sum, g) => sum + g.percentage, 0) / gradeRows.length)
    : 0;
  const gradeStatus = finalGrade >= 85 ? 'A' : finalGrade >= 75 ? 'B+' : finalGrade >= 65 ? 'B' : finalGrade >= 55 ? 'C' : 'D';

  const chartData = [
    { name: 'Class Avg', value: Number(course?.class_average ?? 0) },
  ...
  const modules = (course?.modules || []).map((m, idx) => ({
    ...
    items: (m.items || m.resources || []).map((it, j) => ({
      id: it.id || `${idx + 1}-${j + 1}`,
      type: it.type || 'pdf',
      title: it.title || it.name || 'Resource',
      completed: Boolean(it.completed),
    })),
  }));

  const classmates = (course?.classmates || course?.students || []).map((s, idx) => ({
    id: s.id || idx + 1,
    name: s.name || s.full_name || `${s.first_name || ''} ${s.last_name || ''}`.trim() || 'Student',
    rollNo: s.rollNo || s.roll_no || s.student_id || 'N/A',
  ...
  const handlePostAnnouncement = async () => {
    ...
    try {
      ...
      const payload = {
        title: `Course Update - ${course?.course_code || course?.code || id}`,
        content,
        course_id: Number(id),
      };
      ...
      setNewAnnouncement({ type: 'success', text: content });
      setPostContent('');
    } catch (err) {
      setNewAnnouncement({
        type: 'error',
        text: err.response?.data?.detail || 'Failed to post announcement.',
      });
    } finally {
  ...
  return (
    ...
    <Box className="page-container">
      ...
      {newAnnouncement && (
        <Alert 
          severity={newAnnouncement.type === 'error' ? 'error' : 'info'}
          icon={<Notifications />}
          sx={{ mb: 2, animation: 'slideUp 0.5s ease' }}
        >
          <Typography variant="body2" fontWeight="bold">
            {newAnnouncement.type === 'error'
              ? newAnnouncement.text
              : `New announcement posted in ${course?.title || 'this course'}`}
          </Typography>
        </Alert>
      )}
                    ...
                    <CardContent>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
                        ...
                        <Button
                          ...
                          disabled={!postContent.trim() || postingAnnouncement}
