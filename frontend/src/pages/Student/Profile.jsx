...
const Profile = () => {
  ...
  // Form data — initialized from auth user
  ...
  const buildFormData = (source = {}) => ({
    fullName: source.fullName || source.name || [source.firstName, source.lastName].filter(Boolean).join(' ') || currentUser?.name || '',
    rollNo: source.rollNo || source.roll_no || currentUser?.rollNo || '',
    cnic: source.cnic || '',
    dob: source.dob || '',
    ...
    personalEmail: source.personalEmail || source.email || currentUser?.email || '',
  ...
  const handleSave = () => {
    ...
    const studentPayload = {
      ...
      currentSemester: formData.currentSemester || displayUser?.currentSemester || displayUser?.current_semester || null,
    };
    ...
    Promise.all([
      ...
      .then(async () => {
        ...
        setFormData(buildFormData(refreshed.data || {}));
        setIsEditing(false);
        setHasChanges(false);
        setSnackbar({ open: true, message: 'Profile updated successfully!', severity: 'success' });
      })
      .catch(() => {
        setSnackbar({ open: true, message: 'Unable to save profile changes', severity: 'error' });
      });
  };

  const handleCancel = () => {
    setFormData(buildFormData(profile || currentUser || {}));
    setIsEditing(false);
    setHasChanges(false);
    setErrors({});
  };
  ...
  const handleAvatarUpload = (e) => {
    ...
    if (file) {
      setSnackbar({
        open: true,
        message: 'Profile photo upload endpoint is not configured yet. Please contact support.',
        severity: 'warning',
      });
  ...
  const handleDocumentUpload = () => {
    ...
    setSnackbar({
      open: true,
      message: 'Document upload endpoint is not configured yet. Please contact support.',
      severity: 'warning',
    });
  };
  ...
  const getRiskStatus = () => {
    const cgpa = Number(displayUser?.cgpa || 0);
              ...
              <Grid container spacing={3} alignItems="center">
                ...
                <Grid size={{ xs: 12, sm: 7 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 2, mb: 1, flexWrap: 'wrap' }}>
                    <Chip icon={<BadgeIcon />} label={displayUser?.rollNo} />
                    <Chip icon={<School />} label={displayUser?.program} color="primary" />
                  </Box>
                  <Box sx={{ display: 'flex', gap: 2, mb: 1, flexWrap: 'wrap' }}>
            ...
            <CardContent sx={{ p: { xs: 2, md: 3 } }}>
              ...
              <Grid container spacing={3}>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                  ...
                  <Typography variant="body1" fontWeight="bold">{displayUser?.program}</Typography>
                </Grid>
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
