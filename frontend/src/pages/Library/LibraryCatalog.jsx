...
const LibraryCatalog = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [booksRes, issuedRes, reservedRes] = await Promise.allSettled([
          libraryAPI.searchBooks(),
          libraryAPI.getMyIssues(),
          libraryAPI.getMyReservations(),
        ]);
        setLibraryBooks(booksRes.status === 'fulfilled' ? (booksRes.value.data?.books || booksRes.value.data || []) : []);
        setMyIssuedBooks(issuedRes.status === 'fulfilled' ? (issuedRes.value.data?.issues || issuedRes.value.data || []) : []);
        setMyReservedBooks(reservedRes.status === 'fulfilled' ? (reservedRes.value.data?.reservations || reservedRes.value.data || []) : []);
  ...
  // Calculate stats
  const totalBooks = libraryBooks.reduce((sum, book) => sum + (book.totalCopies || 0), 0);
  const availableBooks = libraryBooks.reduce((sum, book) => sum + (book.availableCopies || 0), 0);
  ...
  const handleReserve = async (bookId) => {
    try {
      await libraryAPI.reserveBook({ book_id: bookId });
      ...
      setMyReservedBooks(res.data?.reservations || res.data || []);
    } catch (e) {
      setSnackbar({ open: true, message: e.response?.data?.detail || 'Reservation failed', severity: 'error' });
    }
  };

  const handleReturn = async (issuedBookId) => {
    try {
      await libraryAPI.returnBook(issuedBookId);
      setSnackbar({ open: true, message: 'Book returned successfully!', severity: 'success' });
      // Refresh issued books
      const res = await libraryAPI.getMyIssues();
      setMyIssuedBooks(res.data?.issues || res.data || []);
    } catch (e) {
      setSnackbar({ open: true, message: e.response?.data?.detail || 'Return failed', severity: 'error' });
    }
  };

  const handleRenew = async (issuedBookId) => {
    try {
      await libraryAPI.returnBook(issuedBookId, { action: 'renew' });
      setSnackbar({ open: true, message: 'Book renewed successfully!', severity: 'success' });
      const res = await libraryAPI.getMyIssues();
      setMyIssuedBooks(res.data?.issues || res.data || []);
    } catch (e) {
      setSnackbar({ open: true, message: e.response?.data?.detail || 'Renew failed', severity: 'info' });
    }
  };
  ...
  const handleCancelReservation = async (reservationId) => {
    try {
      ...
      setSnackbar({ open: true, message: e.response?.data?.detail || 'Cancel failed', severity: 'error' });
    }
  };
