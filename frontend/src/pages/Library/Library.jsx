...
const Library = () => {
  ...
  const [quickReserveBook, setQuickReserveBook] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAllBooks(booksRes.status === 'fulfilled' ? (booksRes.value.data?.books || booksRes.value.data || []) : []);
  ...
  const libraryBooks = allBooks;

  const categories = useMemo(() => {
    const unique = new Set(allBooks.map((b) => b.category));
  ...
  const confirmReservation = async () => {
    try {
      await libraryAPI.reserveBook({ book_id: selectedBook.id || selectedBook.book_id });
      showSnackbar('Book reserved successfully!', 'success');
      // Re-fetch data to update stats and availability
      const [booksRes, issuedRes, reservedRes] = await Promise.allSettled([
        libraryAPI.searchBooks(),
        libraryAPI.getMyIssues(),
        libraryAPI.getMyReservations(),
      ...
      showSnackbar(e.response?.data?.detail || 'Reservation failed', 'error');
    }
    setReserveDialog(false);
    setSelectedBook(null);
    setQuickReserveBook(null);
  };
            ...
            >
              <CardContent sx={{ p: 3 }}>
                <Typography variant="h6" fontWeight="bold" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Bookmark color="primary" /> Quick Reserve
                </Typography>
                <Typography variant="caption" color="text.secondary" display="block" sx={{ mb: 2 }}>
                  Already know what you want? Select a book to hold it.
                </Typography>
                
                <Autocomplete
                  fullWidth
                  size="small"
                  options={allBooks.filter(b => b.availableCopies > 0)}
                  getOptionLabel={(option) => `${option.title} (${option.isbn})`}
                  onChange={(e, newValue) => setQuickReserveBook(newValue)}
                  renderInput={(params) => (
                    <TextField {...params} label="Select Book" placeholder="Search by name or ISBN..." />
                  )}
                  sx={{ mb: 2 }}
                />
                ...
                <Button
                  fullWidth
                  variant="contained"
                  disabled={!quickReserveBook || issuedLimitReached}
                  onClick={() => handleReserve(quickReserveBook)}
                  sx={{ 
                    borderRadius: 2,
                    background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
                    fontWeight: 'bold',
