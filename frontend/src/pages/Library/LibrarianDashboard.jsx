...
const LibrarianDashboard = () => {
  ...
  const handleStudentBlur = () => {
    const student = studentsList.find((s) => String(s.id || s.student_id) === String(studentId));
    setStudentName(student?.name || student?.first_name || student?.roll_no || `Student ${studentId}`);
  };
  ...
  const handleIssue = async () => {
    ...
    try {
      ...
      // Refresh data
      const [booksRes, issuesRes] = await Promise.allSettled([
        libraryAPI.searchBooks(),
        libraryAPI.getMyIssues(),
      ]);
      setBooks(booksRes.status === 'fulfilled' ? (booksRes.value.data?.books || booksRes.value.data || []) : books);
      setTransactions(issuesRes.status === 'fulfilled' ? (issuesRes.value.data?.issues || issuesRes.value.data || []) : transactions);
      ...
      showSnackbar(e.response?.data?.detail || 'Issue failed', 'error');
    }
  };

  const handleReturnSearch = () => {
    const loan = transactions.find(
      (t) => t.status === 'issued' && (String(t.isbn || t.bookIsbn || '') === String(returnSearch) || String(t.studentId || '') === String(returnSearch))
    );
  ...
  const handleReturn = async () => {
    ...
    try {
      const res = await libraryAPI.returnBook(activeLoan.id, { condition: bookCondition });
      ...
      setBooks(booksRes.status === 'fulfilled' ? (booksRes.value.data?.books || booksRes.value.data || []) : books);
      setTransactions(issuesRes.status === 'fulfilled' ? (issuesRes.value.data?.issues || issuesRes.value.data || []) : transactions);
      showSnackbar(`Book returned. Fine: PKR ${fine}`, fine > 0 ? 'warning' : 'success');
      setReturnSearch('');
      setActiveLoan(null);
      setBookCondition('Good');
    } catch (e) {
      showSnackbar(e.response?.data?.detail || 'Return failed', 'error');
    }
  };
