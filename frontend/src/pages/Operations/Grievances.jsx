...
const Grievances = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setGrievanceList(res.data?.grievances || res.data || []);
      } catch { /* empty */ }
      setLoading(false);
    };
    fetchData();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await opsAPI.createGrievance({
        ...formData,
        sentimentScore,
      });
      const result = res.data;
      setSnackbar({ open: true, message: `Ticket created! ID: ${result.ticketId || result.ticket_id || 'NEW'}`, severity: 'success' });
      setSubmitModalOpen(false);
      setFormData({ category: '', priority: '', subject: '', description: '', attachments: [] });
      // Refresh
      const refreshRes = await opsAPI.getGrievances();
      setGrievanceList(refreshRes.data?.grievances || refreshRes.data || []);
    } catch {
      setSnackbar({ open: true, message: 'Failed to submit grievance', severity: 'error' });
    }
  };
  ...
  const handleAddComment = async () => {
    ...
    try {
      await opsAPI.addGrievanceComment(selectedGrievance.id, { content: newComment });
      setSnackbar({ open: true, message: 'Comment added', severity: 'success' });
      setNewComment('');
      // Refresh
      const refreshRes = await opsAPI.getGrievances();
      const refreshed = refreshRes.data?.grievances || refreshRes.data || [];
      setGrievanceList(refreshed);
      setSelectedGrievance(refreshed.find(g => g.id === selectedGrievance.id) || null);
    } catch {
      setSnackbar({ open: true, message: 'Failed to add comment', severity: 'error' });
    }
  };

  const handleCloseTicket = async (rating) => {
    try {
      await opsAPI.updateGrievanceStatus(selectedGrievance.id, { status: 'Closed', rating });
      setSnackbar({ open: true, message: 'Ticket closed', severity: 'success' });
      setDetailsModalOpen(false);
      const refreshRes = await opsAPI.getGrievances();
      setGrievanceList(refreshRes.data?.grievances || refreshRes.data || []);
    } catch {
      setSnackbar({ open: true, message: 'Failed to close ticket', severity: 'error' });
    }
  };
