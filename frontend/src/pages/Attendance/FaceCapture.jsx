...
const FaceCapture = () => {
  ...
  // Confidence animation starts once user provides an image for verification.
  useEffect(() => {
    if (cameraActive && selectedImageFile) {
  ...
  const captureFace = async () => {
    if (!selectedImageFile) {
      setCaptureError('Please upload a clear face image before verification.');
      return;
    }

    sessionStorage.setItem('capturedFace', selectedImageUrl);
    try {
      const formData = new FormData();
      formData.append('image', selectedImageFile, selectedImageFile.name || 'face-capture.png');
      await attendanceAPI.verifyFace(formData);
      navigate('/attendance/confirmation');
    } catch (e) {
      console.error(e);
      setCaptureError('Face verification failed. Please try again.');
    }
  };
