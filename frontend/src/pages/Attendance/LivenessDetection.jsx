...
const LivenessDetection = () => {
  ...
  const handleCapture = async () => {
    try {
      ...
      formData.append('method', livenessMethod);
      const res = await attendanceAPI.checkLiveness(formData);
      const ok = Boolean(res.data?.liveness_verified || res.data?.success || res.data?.verified);
      setFaceDetected(ok);
      setGoodLighting(ok);
      setLivenessVerified(ok);
      if (!ok) {
        return;
      }
      setTimeout(() => {
