...
const mapBookPayload = (data = {}) => ({
  ...
  publication_year: (data.publication_year && data.publication_year !== '') ? toNumber(data.publication_year, null) : null,
  pages: (data.pages && data.pages !== '') ? toNumber(data.pages, null) : null,
  cover_image: data.cover_image ?? data.coverImage ?? null,
  description: data.description ?? null,
  language: data.language ?? 'English',
  total_copies: toNumber(data.total_copies ?? data.totalCopies, 1),
  available_copies: (() => {
    const val = data.available_copies ?? data.availableCopies;
...
export const libraryAPI = {
  ...
  exportBooks: () => client.get('/library/books/export', { responseType: 'blob' }),
  importBooks: (file) => {
    const formData = new FormData();
    formData.append('file', file);
    return client.post('/library/books/import', formData, {
  ...
  issueBook: async (data) => {
    const payload = {
      student_id: (typeof data.student_id === 'number' || !isNaN(Number(data.student_id))) ? Number(data.student_id) : null,
      user_id: (typeof data.student_id === 'string' && data.student_id.length > 20) ? data.student_id : (data.user_id || null),
  ...
  downloadReport: () => client.get('/library/reports/download', { responseType: 'blob' }),
};
