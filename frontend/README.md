...
|-----------|-----------|---------|
| **API Framework** | FastAPI (Python) | Domain-based REST APIs |
| **Authentication** | JWT + Redis session store | Secure token-based auth |
| **Database (Relational)** | PostgreSQL | SIS, finance, and reference data |
| **Database (Document)** | MongoDB | Chat, notifications, analytics, and ops content |
| **Cache Layer** | Redis | Sessions, caching, and queue support |
| **Vector Database** | ChromaDB | AI embeddings for the assistant pipeline |
| **Message Queue** | Apache Kafka | Event-driven service integration |
| **File Storage** | S3-compatible storage | Document and media storage |
...
Use the seeded accounts from [`infrastructure/seeding/postgres/seed-dev.sql`](../infrastructure/seeding/postgres/seed-dev.sql) when you load the development seed data.

| Role | Email |
|------|-------|
| **Student** | `student@nexus.edu` |
| **Faculty** | `faculty@nexus.edu` |
| **Admin** | `admin@nexus.edu` |
| **HOD** | `hod@nexus.edu` |
| **Librarian** | `librarian@nexus.edu` |
| **Alumni** | `alumni@nexus.edu` |
...
│   │       └── Grievances.jsx
│   │
│   ├── 📂 data/                    # API helpers and derived state
│   │
...
| **Phase 2** | ✅ Completed | Backend API integration and role-specific flows |
| **Phase 3** | ✅ Completed | AI, biometric, and analytics features |
| **Phase 4** | ✅ Completed | Deployment, validation, and packaging |
...
- Role-specific frontend pages now use backend-backed profile, transcript, assignment, notification, grievance, alumni, and library data.
- API normalization is centralized in `src/api/*`, with a dedicated `studentAPI` for student-facing pages.
- Teacher and librarian profile pages persist to backend storage instead of relying on UI-only state.
