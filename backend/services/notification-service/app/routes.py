...
async def create_notification(
    ...
    await redis_client.publish(f"notify:{payload.user_id}", json.dumps({"id": notif_id, **doc}))
...
async def create_global_announcement(
    ...
    await redis_client.publish("notify:broadcast", json.dumps({"event": "global_announcement", **doc}))
