...
| Redis | `redis.asyncio` — notification queue & feature flags |
...
| `REDIS_URL` | No | `redis://redis:6379` | Redis connection (async) |
...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| PUT | `/api/v1/ops/feature-flags/{name}` | admin | Create/update feature flag in Redis |
| GET | `/api/v1/ops/feature-flags/{name}` | Any user | Check if feature flag is enabled (role-aware) |
| GET | `/api/v1/ops/feature-flags` | admin | List all feature flags via `SCAN` |
| DELETE | `/api/v1/ops/feature-flags/{name}` | admin | Delete a feature flag |
