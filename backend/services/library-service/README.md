...
> Digital library management — book catalog, issue/return with overdue fines, reservations with auto-expiry, QR code generation, and reporting dashboard.
...
### Book Catalog (5 endpoints)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/v1/library/books` | None | Search catalog by `title`, `author`, `isbn` (pagination: `skip`/`limit`) |
| GET | `/api/v1/library/books/{book_id}` | None | Get single book by ID |
| POST | `/api/v1/library/books` | admin, librarian | Add a new book (409 if ISBN duplicate) |
| PUT | `/api/v1/library/books/{book_id}` | admin, librarian | Partial update of book fields |
| DELETE | `/api/v1/library/books/{book_id}` | admin, librarian | Delete book (blocked if active issues exist) |

### Issue/Return (3 endpoints)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| POST | `/api/v1/library/issues` | admin, librarian | Issue a book to a student (enforces MAX_ACTIVE_ISSUES) |
| POST | `/api/v1/library/returns/{issue_id}` | admin, librarian | Process return; calculates fine and writes to `fin_fines` |
| GET | `/api/v1/library/issues/me` | Any user | Get current student's issue history |

### QR Code (1 endpoint)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/v1/library/qr/{student_id}` | Any user | Generate QR code PNG for student ID |

### Reservations (4 endpoints)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| POST | `/api/v1/library/reservations` | Any user | Reserve a book (3-day expiry; 409 if already reserved) |
| GET | `/api/v1/library/reservations` | admin, librarian | List all active reservations |
| GET | `/api/v1/library/reservations/me` | Any user | Get current student's reservations |
| DELETE | `/api/v1/library/reservations/{reservation_id}` | Any user | Cancel a reservation |

### Reports (1 endpoint)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/v1/library/reports` | admin, librarian | Dashboard stats: totals, books by category, recent 10 transactions |
