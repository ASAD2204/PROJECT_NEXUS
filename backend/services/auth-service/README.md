...
| 12 | `PUT` | `/users/{user_id}` | Admin | Update a user's account details |
| 13 | `DELETE` | `/users/{user_id}` | Admin | Permanently delete a user |
| 14 | `GET` | `/users/by-email/{email}` | No | Lookup a user's basic info by email |
| 15 | `GET` | `/health` | No | Health check (Status of service, DB, and Redis) |
...
|------------|-------------|
| `get_current_user` | Extracts and validates JWT from Bearer token header, checks the active Redis session, and verifies the account is active. Returns `{user_id, role, email}`. Raises 401/403 when invalid, logged out, or deactivated. |
