...
async def faculty_dashboard(
    ...
    for section in sections:
        ...
        # Avg assignment score (graded submissions)
        avg_assign_result = (
            db.query(func.avg(LmsSubmission.marks_obtained / LmsAssignment.total_marks * 100))
            .join(LmsAssignment)
            .filter(
                LmsAssignment.section_id == section.section_id,
                LmsSubmission.marks_obtained.isnot(None)
            )
            .scalar()
        )
        avg_assign_pct = float(avg_assign_result) if avg_assign_result else 0.0
        ...
        pending_assign = (
            ...
            .filter(
                LmsAssignment.section_id == section.section_id,
                LmsSubmission.marks_obtained.is_(None)
            )
            .count()
        )
        total_pending += pending_assign

        # At risk count (attendance < 75%)
        at_risk = (
            db.query(SisStudent)
            .join(SisEnrollment, SisEnrollment.student_id == SisStudent.student_id)
            .filter(SisEnrollment.section_id == section.section_id)
            .filter(SisStudent.current_risk_status == "Red")
            .count()
        )

        course_name = section.course.title if section.course else f"Section {section.section_id}"
        ...
        section_summaries.append(
            SectionPerformanceSummary(
                ...
                avg_assignment_score=round(avg_assign_pct, 2),
...
async def student_risk(
    ...
    if role == "student":
        ...
        if student.student_id != student_id:
            raise HTTPException(status_code=403, detail="Students can only view their own risk profile")
