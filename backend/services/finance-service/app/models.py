from sqlalchemy import Column, Integer, String, ForeignKey, TIMESTAMP, Date, Numeric, Float
...
class SisProgram(Base):
    ...
    department = relationship("SisDepartment")
...
class SisStudent(Base):
    ...
    program = relationship("SisProgram")
...
class FinFeeStructure(Base):
    ...
    struct_id = Column(Integer, primary_key=True, autoincrement=True)
    dept_id = Column(Integer, ForeignKey("sis_departments.dept_id"), nullable=True)
    program_id = Column(Integer, ForeignKey("sis_programs.program_id"), nullable=True)
