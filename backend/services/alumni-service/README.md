...
### Registration & Directory (3 endpoints)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| POST | `/api/v1/alumni/register` | alumni | Register as alumni; validates student exists; prevents duplicates (409) |
| GET | `/api/v1/alumni/directory` | Any user | Browse alumni with optional `?grad_year=` and `?employer=` filters |
| GET | `/api/v1/alumni/{alumni_id}` | Any user | Get single alumni profile (catch-all route, placed last) |

### Profile (1 endpoint)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| PUT | `/api/v1/alumni/profile` | alumni | Update own alumni profile (partial update via `exclude_unset`) |

### Job Board (3 endpoints)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/v1/alumni/jobs` | Any user | List active + approved job postings (sorted by `posted_at` desc) |
| POST | `/api/v1/alumni/jobs` | alumni | Post a job (starts as `"Pending"`); resolves alumni via JWT chain |
| PUT | `/api/v1/alumni/jobs/{job_id}/approve` | admin | Approve a pending job posting |

### Events (3 endpoints)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/v1/alumni/events` | Any user | List all events (sorted by `event_date` desc) |
| POST | `/api/v1/alumni/events` | admin, alumni | Create a new event; stores `created_by` from JWT |
| POST | `/api/v1/alumni/events/{event_id}/register` | Any user | Register for an event; prevents duplicates (409) |

### Mentorship (2 endpoints)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/v1/alumni/mentorship` | Any user | List all mentorship entries (sorted by `created_at` desc) |
| POST | `/api/v1/alumni/mentorship` | alumni | Offer mentorship (one profile per alumni) |

### Success Stories (3 endpoints)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/v1/alumni/stories` | Any user | List **approved** success stories only |
| POST | `/api/v1/alumni/stories` | alumni | Submit a success story (starts as `"Pending"`) |
| PUT | `/api/v1/alumni/stories/{story_id}/approve` | admin | Approve a success story |
