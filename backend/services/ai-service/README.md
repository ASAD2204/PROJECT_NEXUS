...
| **Redis** | Semantic cache (CAG engine) and auth session validation |
...
└── app/
    ...
    ├── config.py              # Settings & environment variables
        ...
        ├── dependencies.py        # JWT + Redis session auth dependency
...
|------------|----------|-------------|
| `chat_messages` | `nexus_chat` | Individual chat messages (user & assistant) |
| `chat_sessions` | `nexus_chat` | Chat session tracking |
...
| 3 | `DELETE` | `/chat/history` | Yes | Clear chat history for the current user, or a single session when `session_id` is provided |
