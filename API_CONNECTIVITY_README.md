...
| enrollFace | POST /attendance/enroll | Not present in current backend routes |
...
| getInvoices | GET /finance/invoices | Not present in current backend routes |
| getInvoice | GET /finance/invoices/{id} | Backend expects student_id path, not invoice_id lookup |
| createInvoice | POST /finance/invoices | Not present in current backend routes |
...
| getFines | GET /finance/fines | Not present in current backend routes |
